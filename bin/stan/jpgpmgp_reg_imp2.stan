...
data {
 // Sample Size
  ...
  int<lower=1> N2;
  // Arrays of location coordinates
  array[N1] vector[2] loc1;
  array[N2] vector[2] loc2;
  // Number of covariates (excluding intercept)
  int<lower=0> P;
  
  // Matrix of missing indicators (R[i,j] == 1 => missing); 
  // Column 1 is theta; Column 2 is Y
  matrix<lower=0,upper=1>[N1, 2] R;

  // Matrix of N observations of r angular components
  vector[N1] theta1;
  // Matrix of N observations of q inline components
  vector[N1] Y1;
  // Matrix of N observations of K covariates
  matrix[N1, P] X1;
}
...
transformed data{
 ...
 // Construct vector to store missing data indices
  int N_mis_theta = to_int(2*sum(R[,1]));
  int N_mis_y = to_int(sum(R[,2]));
  int N_mis = N_mis_theta + N_mis_y;
  int N_obs_theta = N1 - to_int(sum(R[,1]));
...
parameters{
 vector<lower=0>[N_obs_theta] latent_lengths;
 vector[2] mu0;
 ...
 real<lower=0> alpha_y;
  ...
  matrix[N_mis_theta, 2] W1_miss;
  vector[N_mis_y] Y1_miss;
...
transformed parameters {
  ...
  // Missing Data
    ...
    if (N_mis > 0) {
      ...
      int id = 1;
      int iy = 1;
      // Vector Z_miss stores missing data draws
      for (i in 1:N1) {
        if (R[i,1] == 1) {
          W1[i, ] = W1_miss[it,];
          it += 1;
...
model{
    ...
    vector[N_obs_theta] lengths_llpd;
    {
      real a;
      real b;
      int id = 1;
      ...
      for (i in 1:N1) {
        ...
        // E_si = rep_vector(1.0, 2);

        a = quad_form(Sigma_inv, U1[i,]');
        b = U1[i,] * Sigma_inv * E_si;
        lengths_llpd[id] = log(latent_lengths[id]) - K_inv[i,i] / 2.0 * a * (latent_lengths[id] - b / a)^2;
        id += 1;
        }
      }
    }
    ...
    for (i in 1:N1) {
      W1[i,]' ~ multi_normal_cholesky(mu_mat_w[i,]' + f[i,]', L_Sigma);
    }
    ...
    matrix[N1, N1] L_K_y = build_cholesky_factor(Yl1, loc[1:N1,], alpha_y, rho_y, sigma_y);
    
    vector[N1] mu_y = beta_y[1] + X1 * beta_y[2:(P+1)] + W1 * beta_y[(P+2):(P+3)];

    Yl1 ~ multi_normal_cholesky(mu_y, L_K_y);
}
